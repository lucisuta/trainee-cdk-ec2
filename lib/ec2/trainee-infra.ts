...
export class TraineeInfrastructure extends Construct {
	constructor(scope: Construct, id: string, props: TraineeInfrastructureProps) {
		...
		const vpc = new ec2.Vpc(this, 'trainee-vpc-ec2', {
			vpcName: 'trainee-vpc-ec2'
		});

		// security group for instances
		const sg = new ec2.SecurityGroup(this, 'trainee-sg-ec2', {
			securityGroupName: 'trainee-sg-ec2',
			vpc: vpc,
		...
		const instanceRole = new iam.Role(this, 'trainee-instance-role-ec2', {
			roleName: 'trainee-instance-role-ec2',
		...
		const template = new ec2.LaunchTemplate(this, 'trainee-launch-template-ec2', {
			launchTemplateName: "trainee-ec2",
